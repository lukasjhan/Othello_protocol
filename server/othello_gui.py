...
DEFAULT_ROWS = 8
DEFAULT_COLUMNS = 8
DEFAULT_FIRST_PLAYER = othello_set.BLACK
DEFAULT_TOP_LEFT_PLAYER = othello_set.WHITE
DEFAULT_VICTORY_TYPE = othello_set.LEAST_CELLS
...
class OthelloGUI:
    def __init__(self):
        ...
        self._game_state = othello_set.OthelloGame(self._rows, self._columns,
        ...
        self._black_score = othello_models.Score(othello_set.BLACK, self._game_state, self._root_window)
        self._white_score = othello_models.Score(othello_set.WHITE, self._game_state, self._root_window)
    ...
    def _configure_game_settings(self) -> None:
        ...
        if dialog.was_ok_clicked():
            ...
            self.gameserver = othello_game_server.othello_network_server(self.port, self.queue)
    ...
    def _new_game(self) -> None:
        ...
        self._root_window.after(10, self.check_move, self.queue)
        
    def check_move(self, q):
        try:
            row, col = q.get(0)
        except queue.Empty:
            self._root_window.after(10, self.check_move, q)
        try:
